...
Canvas border_canvas(Canvas& canvas, BorderContext border_context, Context context) {
    Canvas cnvs;
    init(cnvs, canvas.height + 2, canvas.width + 2, context);

    draw_canvas_into_canvas(cnvs, 1, 1, canvas);

    for (usize i = 0; i < canvas.width; i++) {
        set(cnvs, 0, i + 1, border_context.horizontal);
        set(cnvs, cnvs.height - 1, i + 1, border_context.horizontal);
    }

    for (usize i = 0; i < canvas.height; i++) {
        set(cnvs, i + 1, 0, border_context.vertical);
        set(cnvs, i + 1, cnvs.width - 1, border_context.vertical);
    }
    set(cnvs, 0, 0, border_context.top_left);
    set(cnvs, 0, cnvs.width - 1, border_context.top_right);
    set(cnvs, cnvs.height - 1, 0, border_context.bottom_left);
    set(cnvs, cnvs.height - 1, cnvs.width - 1, border_context.bottom_right);
