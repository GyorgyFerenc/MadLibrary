...
namespace Stack_ {
template <class T>
inline void init(Stack<T>* stack, Context context) {
    stack->allocator = context.allocator;
}

template <class T>
inline Stack<T> create(Context context) {
    Stack<T> stack;
    init(&stack);
    return stack;
}
...
void push(Stack<T>* stack, T element) {
    let try_node = IAllocator_::allocate<typename Stack<T>::Node>(stack->allocator);
    let node = Errorable_::unwrap(try_node, "stack push");
    node->elem = element;
