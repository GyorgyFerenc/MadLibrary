...
void init(List<T>* list, Context context) {
    ...
    list->size = 0;
    list->capacity = 1;
    let try_ptr = IAllocator_::allocate_array<T>(list->allocator, 1);
    list->ptr = Errorable_::unwrap(try_ptr);
}
...
void destroy(List<T>* list) {
    IAllocator_::free_array(list->allocator, list->ptr, list->capacity);
}
...
List<T> clone(List<T> list, Context context) {
    ...
    let try_ptr = IAllocator_::allocate_array<T>(new_list.allocator, list.capacity);
    new_list.ptr = Errorable_::unwrap(try_ptr);
...
void reserve(List<T>* list, usize new_capacity) {
    ...
    let try_ptr = IAllocator_::allocate_array<T>(list->allocator, new_capacity);
    let ptr = Errorable_::unwrap(try_ptr, "upsize");
...
Array<T> to_array(List<T> list, Context context) {
    let array = Array_::create<T>(list.size, context);
...
void reverse(List<T>* list) {
    for (usize i = 0; i < list->size / 2; i++) {
        T temp = list->ptr[i];
        list->ptr[i] = list->ptr[list->size - i - 1];
        list->ptr[list->size - i - 1] = temp;
    }
}
...
namespace List_ {
/*
 * It needs the address of list as a reference
 */
template <class T>
